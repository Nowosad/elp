...
```{r faithful, echo=FALSE, fig.cap="Obraz Alberta Bierstadta przedstawiający gejzer Old Faithful około roku 1881. Źródło: [Wikipedia](https://commons.wikimedia.org/wiki/File:Bierstadt_Albert_Old_Faithful.jpg)."}
...
4) Pobierz plik `` znajdujący się pod adresem ``.
Rozpakuj go do podfolderu `"data"` i usuń to archiwum z poziomu R.

5) Wczytaj dane z pliku `` xls.
Usuń z nowego obiektu pierwszą kolumnę i zapisz go jako plik w binarnym formacie RDS.

6) Wczytaj plik `` oraz `` do R.
...
8) Napisz funkcję, która wczyta plik tekstowy ``, a następnie zwróci odwrotność jego zawartość, tj. pierwsza linia tekstu w pliku wejściowym ma być ostatnią linią tekstu w przetworzonym obiekcie, a pierwszy znak w danej linii ma stać się ostatnim, itd.
