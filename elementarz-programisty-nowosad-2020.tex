...
Wynik, 18363, oznacza liczbę dni od 1970-01-01.\footnote{\url{https://en.wikipedia.org/wiki/Unix_time}}
...
\CommentTok{#> 1 mi_do_km1(odl_mile)   1.29us 1.43us   611481.}
\CommentTok{#> 2 mi_do_km2(odl_mile) 942.03ns 1.04us   900797.}
...
\CommentTok{#> 1 mi_do_km1(odl_mile2) 383ms  386ms      2.59     382MB}
\CommentTok{#> 2 mi_do_km2(odl_mile2) 705us  740us   1235.      78.2KB}
...
Funkcja \texttt{mi\_do\_km1} jest w stanie wykonać tylko 2.59 operacji na sekundę, przy aż 1235.46 operacji na sekundę funkcji \texttt{mi\_do\_km2}.
...
\hypertarget{evalcpp}{%
\subsection{evalCpp}\label{evalcpp}}

\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{evalCpp}\NormalTok{(}\StringTok{''}\NormalTok{)}
\end{Highlighting}
\end{Shaded}

\hypertarget{cppfunction}{%
\subsection{cppFunction}\label{cppfunction}}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{rcpp_fun =}\StringTok{ "List mi_do_km3(List odl_mile)\{}
...
\StringTok{\}"}
\NormalTok{Rcpp}\OperatorTok{::}\KeywordTok{cppFunction}\NormalTok{(rcpp_fun)}
\end{Highlighting}
\end{Shaded}
...
\begin{Highlighting}[]
\NormalTok{odl_mile =}\StringTok{ }\KeywordTok{list}\NormalTok{(}\DecValTok{142}\NormalTok{, }\DecValTok{63}\NormalTok{, }\DecValTok{121}\NormalTok{)}
\KeywordTok{mi_do_km3}\NormalTok{(odl_mile)}
\CommentTok{#> [[1]]}
\CommentTok{#> [1] 228}
\CommentTok{#> }
\CommentTok{#> [[2]]}
...
\NormalTok{wynik =}\StringTok{ }\NormalTok{bench}\OperatorTok{::}\KeywordTok{mark}\NormalTok{(}
  \KeywordTok{mi_do_km2}\NormalTok{(odl_mile2),}
  \KeywordTok{mi_do_km3}\NormalTok{(odl_mile2)}
\NormalTok{)}
\NormalTok{wynik}
\CommentTok{#> # A tibble: 2 x 6}
\CommentTok{#>   expression             min median `itr/sec` mem_alloc}
\CommentTok{#>   <bch:expr>           <bch> <bch:>     <dbl> <bch:byt>}
\CommentTok{#> 1 mi_do_km2(odl_mile2) 720us  763us     1264.   599.2KB}
\CommentTok{#> 2 mi_do_km3(odl_mile2) 384us  419us     2383.    85.9KB}
\CommentTok{#> # ... with 1 more variable: `gc/sec` <dbl>}
\end{Highlighting}
\end{Shaded}

\hypertarget{cpp-1}{%
\subsection{.cpp}\label{cpp-1}}
...
\KeywordTok{using} \KeywordTok{namespace}\NormalTok{ Rcpp;}
\CommentTok{// [[Rcpp::export]]}
\NormalTok{List mi_do_km3(List odl_mile)\{}
...
\begin{Highlighting}[]
\KeywordTok{library}\NormalTok{(Rcpp)}
\KeywordTok{sourceCpp}\NormalTok{(}\StringTok{"mi_do_km3.cpp"}\NormalTok{)}
