...
\CommentTok{#> 1 mi_do_km1(odl_mile)  1.5us 1.71us   499328.     117KB}
\CommentTok{#> 2 mi_do_km2(odl_mile) 1.16us 1.32us   667601.     224KB}
...
\CommentTok{#> 2 mi_do_km2(odl_mile2) 820us  845us   1023.      78.2KB}
\CommentTok{#> # ... with 1 more variable: `gc/sec` <dbl>}
\end{Highlighting}
\end{Shaded}

W tym przypadku różnica pomiędzy \texttt{mi\_do\_km1} a \texttt{mi\_do\_km2} staje się dużo większa.
Funkcja \texttt{mi\_do\_km1} jest w stanie wykonać tylko 2 operacji na sekundę, przy aż 1022.8 operacji na sekundę funkcji \texttt{mi\_do\_km2}.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{fun1}\NormalTok{(}\DecValTok{1}\NormalTok{)}
\CommentTok{#> [1] 25.4}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{fun1}\NormalTok{(}\OperatorTok{-}\DecValTok{1}\NormalTok{)}
...
\CommentTok{#> No traceback available}
\end{Highlighting}
\end{Shaded}

Debug -\textgreater{} On Error -\textgreater{} Error Inspector
...
Teraz w oknie Environment można znaleźć dwie grupy informacji: istniejące obiekty oraz Traceback zawierający \ldots{} .
...
\CommentTok{#> 1 mile_na_km(odl_mile2)     820us  842us     1123.}
\CommentTok{#> 2 mile_na_km_cpp(odl_mile2) 420us  452us     2154.}
