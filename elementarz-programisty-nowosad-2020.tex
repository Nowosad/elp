...
\CommentTok{#> 1 mi_do_km1(odl_mile) 1.46us 1.67us   505915.     117KB}
\CommentTok{#> 2 mi_do_km2(odl_mile) 1.19us  1.4us   637169.     224KB}
...
\CommentTok{#>   expression             min median `itr/sec` mem_alloc}
\CommentTok{#>   <bch:expr>           <bch> <bch:>     <dbl> <bch:byt>}
\CommentTok{#> 1 mi_do_km1(odl_mile2) 483ms  484ms      2.07     382MB}
\CommentTok{#> 2 mi_do_km2(odl_mile2) 812us  846us   1005.      78.2KB}
\CommentTok{#> # ... with 1 more variable: `gc/sec` <dbl>}
\end{Highlighting}
\end{Shaded}

W tym przypadku różnica pomiędzy \texttt{mi\_do\_km1} a \texttt{mi\_do\_km2} staje się dużo większa.
Funkcja \texttt{mi\_do\_km1} jest w stanie wykonać tylko 2.07 operacji na sekundę, przy aż 1005.35 operacji na sekundę funkcji \texttt{mi\_do\_km2}.
...
Dodatkowo sprawdza ona czy podana wartość (argument \texttt{x}) jest mniejsza niż zero i w takim wypadku wywołuje ona błąd.
...
\CommentTok{#> Error in inch_to_mm(x): Wartość poniżej 0 nie jest możliwa}
\end{Highlighting}
\end{Shaded}
...
Czytamy jej wynik od dołu do góry, dowiadują się, że wywołanie funkcji \texttt{fun3(-1)} skończyło się błędem wewnątrz funkcji \texttt{inch\_to\_mm(x)}, a dokładnie linią \texttt{stop("Wartość\ poniżej\ 0\ nie\ jest\ możliwa")}.
Posiadając tę wiedzę możemy przejść do funkcji \texttt{inch\_to\_mm()} i zmienić ją zgodnie z naszymi potrzebami.

Alternatywną możliwością do funkcji \texttt{traceback()} jest włączenie opcji interaktywnego debuggera w RStudio.
Pierwszym krokiem jest włączenie tzw. Error Inspectora, co można zrobić w menu \texttt{Debug\ -\textgreater{}\ On\ Error\ -\textgreater{}\ Error\ Inspector}.

\begin{center}\includegraphics[width=\textwidth]{figures/debugger1} \end{center}

Teraz po wykonaniu kodu \texttt{fun3(-1)} otrzymamy nie tylko komunikat błędu, ale także dwie nowe ikony ``Show Traceback''\texttt{i\ "Rerun\ with\ Debug".\ Pierwsza\ z\ nich\ wywołuje\ działanie\ wcześniej\ opisanej\ funkcji}traceback()\texttt{.\ Druga,\ "Rerun\ with\ Debug",\ uruchamia\ interaktywny\ debugger.\ \ W\ tym\ momencie\ cały\ kod\ jest\ ponownie\ uruchamiany\ i\ zatrzymywanyw\ miejscu\ gdzie\ błąd\ powstał.\ \ Teraz\ w\ oknie\ Environment\ można\ znaleźć\ dwie\ grupy\ informacji:\ istniejące\ obiekty\ oraz\ Traceback\ pokazujący\ w\ krórym\ kroku\ obliczeń\ jesteśmy.\ \ Dodatkowo\ konsola\ R\ wygląda\ inaczej\ niż\ zwykle.\ \ Pod\ informacją\ o\ błędzie\ wyświetił\ się\ tekst}Browse{[}1{]}\textgreater{}`, a nad oknem konsoli pojawił się szereg ikon.
...
\tightlist
\item
  Pierwsza ikona ``Next'', skrót klawiaturowy \texttt{n}, wykonuje kolejny krok obliczeń
\item
  Druga ikona, \texttt{s}, działa podobnie do ``Next'', z tym wyjątkiem, że gdy kolejny krok obliczeń jest funkcją to wtedy pozwala ona na wejście do tej funkcji i sprawdzenie jej interaktywnie
\item
...
\CommentTok{#> 1 mile_na_km(odl_mile2)     817us  845us     1142.}
\CommentTok{#> 2 mile_na_km_cpp(odl_mile2) 418us  449us     2169.}
