...
Przykładowo, hierarchiczne formaty danych można wczytać używając pakietu **jsonline** (format `.json`) czy **xml2** (format `.xml`), a formaty danych przestrzennych używając pakietu **sf** (dane wektorowe) czy **raster** (dane rastrowe).
...
Dostęp do baz danych w R możliwy jest używając pakietu **DBI** wraz z dodatkowym pakietem dla konkretnego systemu bazodanowego (np. **RPostgreSQL** dla baz PostgreSQL czy **RSQLite** dla baz SQLite). ^[Więcej informacji o łączeniu się z bazami danych można znaleść na stronie https://db.rstudio.com/getting-started/connect-to-database.]
