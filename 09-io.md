...
Przykładowo, hierarchiczne formaty danych można wczytać używając pakietu **jsonline** (format `.json`) [@R-jsonline] czy **xml2** (format `.xml`) [@R-xml2], a formaty danych przestrzennych używając pakietu **sf** (dane wektorowe) [@R-sf] czy **raster** (dane rastrowe) [@R-raster].
