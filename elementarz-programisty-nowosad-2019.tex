...
Oprócz informacji ściśle powiązanych z R, \textbf{Elementarz programisty} ma także rozdział poświęcony systemom kontroli wersji - uniweralnym narzędziom używanym przez programistów różnych języków.
...
W niektórych rodziałach użyte zostanie zintegrowane środowisko programistyczne \textbf{RStudio}.
...
Zalecanym sposobem installacji Git na Windows jest wersja ze strony \url{https://gitforwindows.org/}.
...
Współcześnie programowanie pozwala na wykonywanie trylionów (10\textsuperscript{18}) operacji arytmetycznych na sekundę\footnote{Przecięty człowiek jest w stanie wykonać około pół operacji na sekundę - \url{https://en.wikipedia.org/wiki/Computer_performance_by_orders_of_magnitude}.}.
...
Inną cechą programowania jest też jego prosta możliwość automatyzacji powtarzanych czynnośći oraz ułatwiona powtarzalność (ang. \emph{reproducibility})
...
Pojawiły się nowe pradygmaty programowania oraz wiele nowych języków.
...
Wsród nich można wymienić działania organizacji \href{https://rladies.org/}{R-Ladies}, \href{https://www.pyladies.com/}{PyLadies}, czy \href{https://girlsjs.pl/}{girls.js}.
...
W poniższych czterech przypadkach obiekt \texttt{x} został przetworzony używając operatorów dodawania, odejmowania, mnożenia oraz dzielenia, a nowe obiekty powstały jako wyniki tych obliczeń.
...
\BeginKnitrBlock{rmdinfo}
Część języków programowania, np. C, wymaga zadeklarowania zmiennej przed jej użyciem poprzez podanie jej nazwy i typu.
...
Operator \texttt{\%/\%} przestawia dzielenie całkowite.
...
W R nie istnieje do tego celu specjalny operator, ale zawiera on specjalną funkcję \texttt{sqrt()}.
...
Te operatory można też użyć jako normalne funkcje poprzez dodanie znaku zwanego grawisem - "\texttt{",\ np.}2 + 2\texttt{można\ też~zapisać\ jako}`+`(2, 2)`.
\EndKnitrBlock{rmdinfo}

\hypertarget{ide}{%
\section{IDE}\label{ide}}

RStudio to zintegrowane środowisko programistyczne (ang. \emph{Integrated Development Environment}, IDE) dla R.
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{miasto_a =}\StringTok{ }\DecValTok{75}
\end{Highlighting}
\end{Shaded}
...
\begin{Shaded}
\begin{Highlighting}[]
\NormalTok{miasto_a_c =}\StringTok{ }\NormalTok{(miasto_a }\OperatorTok{-}\StringTok{ }\DecValTok{32}\NormalTok{) }\OperatorTok{/}\StringTok{ }\FloatTok{1.8}
...
\begin{Highlighting}[]
\NormalTok{miasto_a =}\StringTok{ }\DecValTok{75}
\NormalTok{miasto_b =}\StringTok{ }\DecValTok{110}
\NormalTok{miasto_c =}\StringTok{ }\DecValTok{0}
\NormalTok{miasto_a_c =}\StringTok{ }\NormalTok{(miasto_a }\OperatorTok{-}\StringTok{ }\DecValTok{32}\NormalTok{) }\OperatorTok{/}\StringTok{ }\FloatTok{1.8}
\NormalTok{miasto_b_c =}\StringTok{ }\NormalTok{(miasto_b }\OperatorTok{-}\StringTok{ }\DecValTok{32}\NormalTok{) }\OperatorTok{/}\StringTok{ }\FloatTok{1.8}
\NormalTok{miasto_c_c =}\StringTok{ }\NormalTok{(miasto_c }\OperatorTok{-}\StringTok{ }\DecValTok{32}\NormalTok{) }\OperatorTok{/}\StringTok{ }\FloatTok{1.8}
\end{Highlighting}
\end{Shaded}
...
Zamiast tworzenia skryptu w oparciu o kopiuj/wklej lepiej pomyśleć nad zbudowaniem odpowiedniej funkcji\footnote{\citet{wickham2016r} radzą tworzyć nowe funkcje, gdy ten sam kod potwarza się~co najmniej trzy razy.}.
...
\item
  ...
  W tym pliku, stwórz nowy obiekt \texttt{poznan}, który przyjmuje wartość~\texttt{8.4}, napisz przeliczenie wartości tego obiektu ze stopnii Celsjusza na stopnie Fahrenheita, a następnie wyświetl uzyskany wynik.
  Uwaga: pamiętaj o ustawieniu odpowiedniego kodowania znaków dla tego nowego pliku.
\item
  Stwórz nową funkcję, która służy do przeliczania wartości ze stopnii Celsjusza na stopnie Fahrenheita.
...
W przypadku, gdy ten warunek jest spełniony (czyli jest \texttt{TRUE}), wyświetlany jest tekst \texttt{"Dodatnia"}.
...
\section{Warunki zagnieżdzone}\label{warunki-zagniezdzone}}
...
Wyrażenie \texttt{if()} oczekuje wektora logiczego o długości jeden.
Często jednak efektem porównania może być wektor o większej długości.
Przykładowo, porównanie operatorem \texttt{==} daje w wyniku wektor o długości trzy, a porówanie z użyciem \texttt{\%in\%} skutkuje wektorem o długości jeden.
...
\item
  ...
  Jeżeli wartości gęstości zaludnienia (liczba osób na jednostkę powierchni) jest wyższa niż 123 wyświetl tekst \texttt{"Wartość\ powyżej\ średniej\ dla\ Polski."}.
\end{enumerate}
...
\texttt{Inf} i \texttt{-Inf} (ang. \emph{Infinity}) jest wynikiem obliczeń, które dały bardzo dużą wartość dodatną lub ujemną, przykładowo \texttt{9\^{}999}.
\EndKnitrBlock{rmdinfo}
...
Dodatkowo, posiadają one pewne informacje zaszyte w atrybutach, w tym wartości wszystkich kategorii oraz stwierdzenie posiadanej klasy {[}\^{}Posiadanie attubutu \texttt{class} zamienia je w tak zwane obiekty S3, które zachowują się inaczej niż normalne wektory atomowe.{]}.
...
Wynik, 18048, oznacza liczbę dni od 1970-01-01.\footnote{\url{https://en.wikipedia.org/wiki/Unix_time}}
...
Typ znakowy jest określany poprzez użycie cudzysłowia \texttt{"} lub \texttt{\textquotesingle{}}.
...
R oferuje też uproszczoną postać tej funkcji o nazwie \texttt{paste0()}, w której nie ma znaku seperatora.
...
W powyższym przykładzie oczkiwalibyśmy ułożenia, w których \texttt{"hałas"} byłby na ostatnim miejscu.
...
W tej chwili możmy użyć funkcji \texttt{str\_sort()} jeszcze raz.
...
Teraz \texttt{"hałas"} jest poprawnie na ostanim miejscu, ale na pierwszej pozycji jest \texttt{"ćma"} zamiast \texttt{"czosnek"}.
...
Jego efektem będzie wektor logiczny wskazujący, które elemeny zawierają wybrany wzorzec (\texttt{TRUE}) oraz które go nie zawierają (\texttt{FALSE}).
...
Podaliśmy jako wzorzec wszyskie litery od małego \texttt{a} do małego \texttt{z} oraz dużego \texttt{A} do dużego \texttt{Z}.
...
Zamiast zamiany wszyskich kropek na przecinki, nastąpiła zamiana pierwszego znaku w tekście (litery \texttt{W}) na przecinek.
...
Przykładowo poniższa linia kodu wyświetla wszyskie pliki znajdujące się w folderze \texttt{"pliki"}\footnote{Folder o tej nazwie znajduje się w folderze robocznym.}.
...
Metaznak \texttt{\$} użyty w tym przypadku zapogiega sytuacji, gdy tekst \texttt{.png} znajduje się w środku nazwy pliku.
...
\item
  Posiadasz wektor \texttt{wsp} zawierający współrzedne geograficzne szeregu miast w formacie DMS (Stopnie, Minuty, Sekundy).
...
\BeginKnitrBlock{rmdinfo}
W R istnieją też wielowiarowe obiekty podobne do macierzy zwane matrycami (ang. \emph{array}).
\EndKnitrBlock{rmdinfo}
...
W przypadku macierzy możliwe jest dodatowo sprawdzenie liczby występujących wierszy (\texttt{nrow}) i kolumn (\texttt{ncol}).
...
Domyślnie funkcja \texttt{data.frame} wykonuje jeszcze jedną operację w tle - zamienia ona wszystkie dane o type znakowym na typ czynnikowy (sekcja \ref{fac}).
W większości przypadków nie jest to porządane działanie - dlatego też warto wyłączyć tę konwersję używając argumentu \texttt{stringsAsFactors\ =\ FALSE}.
...
\hypertarget{aczenie-1}{%
\subsection{Łączenie}\label{aczenie-1}}
...
Inną ważną właściwością list jest możliwość ich zagnieżdzania - jedna lista może przechowywać kolejną, która jest w stanie przechowywać następną\ldots{} Z tego powodu listy są czasami nazywane wektorami rekurencyjnymi.
...
Listy są przedstawane jako klasa \texttt{list}.
...
\hypertarget{aczenie-2}{%
\subsection{Łączenie}\label{aczenie-2}}
...
Te funcje rozpoczynają się~od \texttt{as.} a następnie zawierają nazwę klasy do której chcemy przetworzyć wejściowy obiekt, np. \texttt{as.vector()}, \texttt{as.matrix()}, \texttt{as.data.frame()}, \texttt{as.list()}.
...
Natomiat w sytuacji, gdy dla danej klasy obiektu nie ma istniejącej metody, używana jest domyślna metoda (\texttt{.default}).
...
Dlatego w tym przypadku moglibyśmy uniknąć wieloktornego pisania podobnego kodu używając pętli \texttt{for}.
...
\BeginKnitrBlock{rmdinfo}
Tradycyjnie zmienna w pętli \texttt{for} nazywana jest \texttt{i}, a w przypadku zagnieżdzonych pętli druga zmienna nazywana jest \texttt{j}.
...
Inny rodzaj pętli, pętla \texttt{while}, jest natomiast stosowany gdy nie wiadomo ile potwórzeń jest koniecznych.
...
Pętla \texttt{while} składa się z nagłówka definującego pewien warunek oraz ciała określającego operację do wykonania.
...
Pętla \texttt{repeat} powtarza pewnien kod aż do momentu przerwania go przez użytkownika (np. użycie klawisza Esc) lub do pojawienia się~komendy \texttt{break}.
...
Funkcjonał \texttt{lapply()} przyjmuje jako pierwszy arugment wektor atomowy lub listę, a następnie przetwarza go używając funkcji podanej jako drugi argument \texttt{FUN}.
...
Języki programowania mają na celu wykonywanie wielu złożnych operacji w relatywnie krótkim czasie.
...
Posiadając taką więdzę możliwe jest wczytywanie i zapisywanie danych, takich jak dane tekstowe, dane w formatach R, czy dane z arkuszy kalkulacyjnych.
...
\begin{Shaded}
\begin{Highlighting}[]
\KeywordTok{setwd}\NormalTok{(}\StringTok{"home/jakub/Documents/elp/"}\NormalTok{) }\CommentTok{#unix}
...
\hypertarget{zapisywanie-plikow-tekstowych}{%
\section{Zapisywanie plików tekstowych}\label{zapisywanie-plikow-tekstowych}}
...
\hypertarget{part-narzedzia}{%
\part{Narzędzia}\label{part-narzedzia}}
...
W tym przypadku wartość~\texttt{TRUE} zostanie najpierw zamieniona na 1 a \texttt{FALSE} na zero, następnie od tych wartości zostanie odjęte jeden, a na końcu zostaną one zamienione na wartości bezwzględne.
...
Stara się~ona wyliczyć logarytm naturalny (\texttt{log()}) z wartości argumentu \texttt{x}, a w przypadku gdyby napotkała błąd zwóci ona wartość \texttt{NA}.
...
Sprawdżmy to na dwóch przykładach - obiektu \texttt{y} (klasa \texttt{prostokat}) i \texttt{x} (klasa \texttt{matrix}).
...
\CommentTok{#> 1 mi_do_km1(odl_mile) 1.5us 1.68us   501937.     117KB}
\CommentTok{#> 2 mi_do_km2(odl_mile) 1.1us 1.22us   766274.     221KB}
...
\CommentTok{#>   expression             min median `itr/sec` mem_alloc}
\CommentTok{#>   <bch:expr>           <bch> <bch:>     <dbl> <bch:byt>}
\CommentTok{#> 1 mi_do_km1(odl_mile2) 464ms  467ms      2.14     382MB}
\CommentTok{#> 2 mi_do_km2(odl_mile2) 776us  799us   1051.      78.2KB}
\CommentTok{#> # ... with 1 more variable: `gc/sec` <dbl>}
\end{Highlighting}
\end{Shaded}

W tym przypadku różnica pomiędzy \texttt{mi\_do\_km1} a \texttt{mi\_do\_km2} staje się dużo większa.
Funkcja \texttt{mi\_do\_km1} jest w stanie wykonać tylko 17.12 operacji na sekundę, przy aż 19.97 operacji na sekundę funkcji \texttt{mi\_do\_km2}.
...
#> 1 mi_do_km1(l)    10   3.88us   4.38us 211229.  
#> 2 mi_do_km2(l)    10   1.76us   1.97us 474622.  
#> 3 mi_do_km1(l)   100  66.19us  70.56us  13513.  
#> 4 mi_do_km2(l)   100   8.45us   9.45us  97481.  
#> 5 mi_do_km1(l)  1000   4.28ms   4.54ms    216.  
#> 6 mi_do_km2(l)  1000   76.9us  80.67us  11910.  
#> 7 mi_do_km1(l) 10000 532.98ms 532.98ms      1.88
#> 8 mi_do_km2(l) 10000 782.52us 821.74us   1078.  
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# zawierdzenie dodanych zmian}
...
\begin{Shaded}
\begin{Highlighting}[]
\CommentTok{# utworzenienie nowego rozgałęzienia}
...
\hypertarget{dodatkowe-mozliwosci-github}{%
\subsection{Dodatkowe możliwości GitHub}\label{dodatkowe-mozliwosci-github}}
...
\item
  ...
  Dodaj do niego kolejną linię \texttt{Edytowałem\ plik\ z\ poziomu\ GitHub.} oraz napisz odpowiedni komunikat poniżej tego okna i zawierdź zmiany (zielony przycisk \texttt{Commit\ changes}).
  Sprawdź stronę internetową zawierającą zdalne repozytorium ``test''.
  Czy zaszły na niej jakieś zmiany od poprzedniego wejścia?
\item
...
W celu poznania i zrozumienia złożnych aspektów tworzenia pakietów R cennymi źródłami wiedzy może być książki \href{https://r-pkgs.org}{R packages} \citep{wickham2015r} oraz \href{https://ropensci.github.io/dev_guide/}{rOpenSci Packages: Development, Maintenance, and Peer Review} \citep{ropensci_2019_2554759}.
...
\def\labelenumi{\arabic{enumi})}
\item
  Stwórz szkielet nowego pakietu R nazwającego się \textbf{konwerter}.
\item
...
\StringTok{  }\NormalTok{ggplot2}\OperatorTok{::}\KeywordTok{facet_grid}\NormalTok{(zmienna}\OperatorTok{~}\NormalTok{nazwa_stacji, }\DataTypeTok{scale =} \StringTok{"free_y"}\NormalTok{) }\OperatorTok{+}\StringTok{ }
\StringTok{  }
\end{Highlighting}
\end{Shaded}
...
Można to zobaczyć przeglądając tzw. \emph{task views} - listy pakietów zaagregowane według podobnej tematyki znajdujące się pod adresem \url{https://cran.r-project.org/web/views/}.
...
Istnieje wiele książek poświęconych kwestii algorytmów, wsród których najbardziej popularne to Introduction to Algorithms \citep{cormen2009introduction}, The Algorithm Design Manual \citep{skienaAlgorithmDesignManual2008} czy Algorithms \citep{032157351X}.
