...
knitr::include_graphics("images/obiekty.jpg")
```
...
knitr::include_graphics("images/rstudio-ide.png", dpi = "NA")
```
...
```{r rstudiosk, echo=FALSE, warning=FALSE, message=FALSE}
library(tibble)
library(magrittr)
sk = tribble(
  ~`Skrót`, ~`Wyjaśnienie`,
...
```{block2 type="rmdinfo"}
Kodowanie znaków (ang. *character encodings*) jest to sposób prezentacji znaków.
