...
#> 1 mi_do_km1(odl_mile) 1.51µs 1.74µs   481242.     117KB
#> 2 mi_do_km2(odl_mile) 1.17µs 1.38µs   645830.     224KB
...
#>   expression             min median `itr/sec` mem_alloc
#>   <bch:expr>           <bch> <bch:>     <dbl> <bch:byt>
#> 1 mi_do_km1(odl_mile2) 565ms  565ms      1.77     382MB
#> 2 mi_do_km2(odl_mile2) 827µs  861µs    977.      78.2KB
#> # … with 1 more variable: `gc/sec` <dbl>
```

W tym przypadku różnica pomiędzy `mi_do_km1` a `mi_do_km2` staje się dużo większa. 
Funkcja `mi_do_km1` jest w stanie wykonać tylko 1.77 operacji na sekundę, przy aż 976.69 operacji na sekundę funkcji `mi_do_km2`.
