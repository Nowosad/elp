...
#> 1 mi_do_km1(odl_mile) 1.44µs 1.65µs   505876.     117KB
#> 2 mi_do_km2(odl_mile) 1.12µs 1.31µs   670247.     224KB
...
#>   expression             min median `itr/sec` mem_alloc
#>   <bch:expr>           <bch> <bch:>     <dbl> <bch:byt>
#> 1 mi_do_km1(odl_mile2) 560ms  560ms      1.78     382MB
#> 2 mi_do_km2(odl_mile2) 819µs  857µs    976.      78.2KB
#> # … with 1 more variable: `gc/sec` <dbl>
```

W tym przypadku różnica pomiędzy `mi_do_km1` a `mi_do_km2` staje się dużo większa. 
Funkcja `mi_do_km1` jest w stanie wykonać tylko 1.78 operacji na sekundę, przy aż 975.7 operacji na sekundę funkcji `mi_do_km2`.
