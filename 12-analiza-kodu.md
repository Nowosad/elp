...
#> 1 mi_do_km1(odl_mile) 1.46µs 1.72µs   492328.     117KB
#> 2 mi_do_km2(odl_mile) 1.21µs  1.4µs   628654.     224KB
...
#>   expression             min median `itr/sec` mem_alloc
#>   <bch:expr>           <bch> <bch:>     <dbl> <bch:byt>
#> 1 mi_do_km1(odl_mile2) 487ms  492ms      2.03     382MB
#> 2 mi_do_km2(odl_mile2) 811µs  840µs   1017.      78.2KB
#> # … with 1 more variable: `gc/sec` <dbl>
```

W tym przypadku różnica pomiędzy `mi_do_km1` a `mi_do_km2` staje się dużo większa. 
Funkcja `mi_do_km1` jest w stanie wykonać tylko 2.03 operacji na sekundę, przy aż 1017.26 operacji na sekundę funkcji `mi_do_km2`.
