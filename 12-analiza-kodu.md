...
#> 1 mi_do_km1(odl_mile) 1.53µs 1.78µs   472718.     117KB
#> 2 mi_do_km2(odl_mile) 1.23µs 1.46µs   601072.     224KB
...
#>   expression             min median `itr/sec` mem_alloc
#>   <bch:expr>           <bch> <bch:>     <dbl> <bch:byt>
#> 1 mi_do_km1(odl_mile2) 492ms  493ms      2.03     382MB
#> 2 mi_do_km2(odl_mile2) 822µs  854µs    998.      78.2KB
#> # … with 1 more variable: `gc/sec` <dbl>
```

W tym przypadku różnica pomiędzy `mi_do_km1` a `mi_do_km2` staje się dużo większa. 
Funkcja `mi_do_km1` jest w stanie wykonać tylko 2.03 operacji na sekundę, przy aż 997.68 operacji na sekundę funkcji `mi_do_km2`.
