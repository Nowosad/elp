...
#> 1 mi_do_km1(odl_mile) 1.48µs  1.7µs   499853.     117KB
#> 2 mi_do_km2(odl_mile) 1.14µs 1.36µs   651571.     224KB
...
#>   expression             min median `itr/sec` mem_alloc
#>   <bch:expr>           <bch> <bch:>     <dbl> <bch:byt>
#> 1 mi_do_km1(odl_mile2) 517ms  517ms      1.94     382MB
#> 2 mi_do_km2(odl_mile2) 820µs  846µs   1012.      78.2KB
#> # … with 1 more variable: `gc/sec` <dbl>
```

W tym przypadku różnica pomiędzy `mi_do_km1` a `mi_do_km2` staje się dużo większa. 
Funkcja `mi_do_km1` jest w stanie wykonać tylko 1.94 operacji na sekundę, przy aż 1011.95 operacji na sekundę funkcji `mi_do_km2`.
