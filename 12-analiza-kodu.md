...
#> 1 mi_do_km1(odl_mile) 1.51µs 1.71µs   495749.     117KB
#> 2 mi_do_km2(odl_mile) 1.18µs 1.36µs   650595.     224KB
...
#>   expression             min median `itr/sec` mem_alloc
#>   <bch:expr>           <bch> <bch:>     <dbl> <bch:byt>
#> 1 mi_do_km1(odl_mile2) 536ms  536ms      1.86     382MB
#> 2 mi_do_km2(odl_mile2) 813µs  842µs   1003.      78.2KB
#> # … with 1 more variable: `gc/sec` <dbl>
```

W tym przypadku różnica pomiędzy `mi_do_km1` a `mi_do_km2` staje się dużo większa. 
Funkcja `mi_do_km1` jest w stanie wykonać tylko 1.86 operacji na sekundę, przy aż 1002.8 operacji na sekundę funkcji `mi_do_km2`.
