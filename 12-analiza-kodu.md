...
#> 1 mi_do_km1(odl_mile) 1.46µs 1.61µs   525810.     117KB
#> 2 mi_do_km2(odl_mile) 1.14µs 1.27µs   692173.     224KB
...
#>   expression             min median `itr/sec` mem_alloc
#>   <bch:expr>           <bch> <bch:>     <dbl> <bch:byt>
#> 1 mi_do_km1(odl_mile2) 472ms  482ms      2.07     382MB
#> 2 mi_do_km2(odl_mile2) 790µs  825µs   1087.      78.2KB
#> # … with 1 more variable: `gc/sec` <dbl>
```

W tym przypadku różnica pomiędzy `mi_do_km1` a `mi_do_km2` staje się dużo większa. 
Funkcja `mi_do_km1` jest w stanie wykonać tylko 2.07 operacji na sekundę, przy aż 1087.07 operacji na sekundę funkcji `mi_do_km2`.
