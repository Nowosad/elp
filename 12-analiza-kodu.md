...
#> 1 mi_do_km1(odl_mile) 1.47µs  1.7µs   496051.     117KB
#> 2 mi_do_km2(odl_mile) 1.14µs 1.37µs   645470.     224KB
...
#>   expression             min median `itr/sec` mem_alloc
#>   <bch:expr>           <bch> <bch:>     <dbl> <bch:byt>
#> 1 mi_do_km1(odl_mile2) 505ms  505ms      1.98     382MB
#> 2 mi_do_km2(odl_mile2) 826µs  857µs    994.      78.2KB
#> # … with 1 more variable: `gc/sec` <dbl>
```

W tym przypadku różnica pomiędzy `mi_do_km1` a `mi_do_km2` staje się dużo większa. 
Funkcja `mi_do_km1` jest w stanie wykonać tylko 1.98 operacji na sekundę, przy aż 993.91 operacji na sekundę funkcji `mi_do_km2`.
